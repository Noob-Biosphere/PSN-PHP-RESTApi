...
final class RestHttpResponse{
    ...
    public static function json($arrResponseBody, $strHttpStatus = '200', $arrHeader = array()) {
        ...
        if (is_object($arrResponseBody)) {
            $arrResponseBody = RestClassUtils::beanToArr($arrResponseBody, false);
        } else if (is_array($arrResponseBody)) {
            $arrResponseBody = RestClassUtils::subBeanToArr($arrResponseBody, false);
        }
    ...
    public static function jsonErr($strErrorReason, $strCode = RestErrorCode::INVALID_ARGUMENT, $strHttpStatus = '400' , $arrHeader = array(), $arrAppend = array(), $arrReplaceArgs = array()) {
        $arrHeader['Content-Type'] = RestContentType::JSON . ";" . self::getCharset();
        ...
        self::_output($strHttpStatus, $arrHeader, json_encode($arrResponseBody), $arrReplaceArgs);
        die();
    }
    ...
    private static function _output($strHttpStatus, $arrHeader, $strResponseBody = '', $arrReplaceArgs = array()) {
        ...
        $str = RestLangUtils::replace($strResponseBody);
        if (is_array($arrReplaceArgs) && isset($arrReplaceArgs[0])) {
            $str = vsprintf($str, $arrReplaceArgs);
        }
        echo $str;
    }
}