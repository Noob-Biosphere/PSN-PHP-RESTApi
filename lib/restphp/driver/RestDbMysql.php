...
class RestDbMysql {
	...
	private function _debugMsgAdd($strSql){
		$strDebugMessage='<br /><br />MySQL Error: Error NO. - '.$this->db->errno().", Description - ".$this->db->error().'<br /> On execute SQL:'.$strSql;
        RestLog::writeLog('<br /><br />MySQL Error: Error NO. - '.$this->db->errno().", Description - ".$this->db->error().'<br /> On execute SQL:'.$strSql);
        throw new RestException($strDebugMessage, RestErrorCode::DB_ERROR);
	}
	...
	public function select($p_arrParams){
		...
		if ('' == $mixLimit && isset($p_arrParams['page_param'])) {
		    $pPageParam = $p_arrParams['page_param'];
		    $mixLimit = ' limit ' . $pPageParam->getFrom() . ", " . $pPageParam->getLimit();
        }

		$strSql = $this->pSqlBuild()->selectConstruct($strTable, $strOutput, $mxRule, $strOrder, $mixLimit);
